...
| `requirements.html` | 版本需求公示页，展示需求状态、负责人、接口人、关联 PRD 和原型入口。 |
| `data/versions.csv` | 版本信息数据源，一行一个版本，使用中文表头。 |
| `data/requirements.csv` | 需求明细数据源，一行一个需求，通过 `版本ID` 归属到版本。 |
...
`requirements.html` 从 CSV 读取数据，无需改页面结构。

- `data/versions.csv`：维护版本状态、版本号、发版日和版本说明。
- `data/requirements.csv`：维护需求点、模块、接口/验收人、优先级、状态、备注、研发测试分工、PRD 链接和原型链接。
- 多个链接用英文分号 `;` 分隔。
- 字段里包含英文逗号时，需要用英文双引号包裹。
